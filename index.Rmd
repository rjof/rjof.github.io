...
coronavirus = read.csv2('~/Projects/github/coronavirus_dashboard/data/coronavirus_dataset.csv'
...
df_ecuador <- coronavirus %>% dplyr::filter(type == "confirmed", Country.Region == "Ecuador") %>%
  dplyr::group_by(date) %>%
...
df_nicaragua <- coronavirus %>% dplyr::filter(type == "confirmed", Country.Region == "Nicaragua") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
df_canada <- coronavirus %>% dplyr::filter(type == "confirmed", Country.Region == "Canada") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
df_mexico <- coronavirus %>% dplyr::filter(type == "confirmed", Country.Region == "Mexico") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
delta = 1500
mex_confirmed = as.numeric(df [ df$Country.Region == 'Mexico', 'confirmed'])
...
data = coronavirus %>%
    ...
    dplyr::ungroup()
delta = 100
mex_confirmed = as.numeric(data [ data$Country.Region == 'Mexico', 'total_cases'])
data %>%
...
data = coronavirus::coronavirus %>%
    ...
    dplyr::arrange(- confirmed)
delta = 1000
mex_confirmed = as.numeric(data [ data$Country.Region == 'Mexico','confirmed'])
